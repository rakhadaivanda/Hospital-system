# Stage 1: Build aplikasi menggunakan Maven
FROM maven:3.9.4-eclipse-temurin-17-alpine AS builder
WORKDIR /app
# Copy file konfigurasi pom.xml dan kode sumber
COPY pom.xml .
COPY src ./src
# Compile aplikasi (membuat file JAR)
RUN mvn clean package -DskipTests

# Stage 2: Jalankan aplikasi menggunakan Java JRE yang sangat ringan
FROM eclipse-temurin:17-jre-alpine
WORKDIR /app
# Copy hasil file JAR dari Stage 1 ke Stage 2
COPY --from=builder /app/target/system-0.0.1-SNAPSHOT.jar app.jar

# Buka port 8080
EXPOSE 8080

# Set default MongoDB URI
ENV HOSPITAL_MONGODB_URI="mongodb+srv://rakhadaivanda_db_user:pcwRoi9oPED1sv3j@cluster0.ukgyx1n.mongodb.net/hospital_db?retryWrites=true&w=majority&appName=Cluster0"

# Jalankan aplikasinya - paksa URI MongoDB via JVM property (prioritas tertinggi)
ENTRYPOINT ["java", "-Dspring.data.mongodb.uri=mongodb+srv://rakhadaivanda_db_user:pcwRoi9oPED1sv3j@cluster0.ukgyx1n.mongodb.net/hospital_db?retryWrites=true&w=majority&appName=Cluster0", "-Dspring.data.mongodb.database=hospital_db", "-jar", "app.jar"]
